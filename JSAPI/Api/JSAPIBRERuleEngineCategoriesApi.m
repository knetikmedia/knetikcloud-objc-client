#import "JSAPIBRERuleEngineCategoriesApi.h"
...
@interface JSAPIBRERuleEngineCategoriesApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation JSAPIBRERuleEngineCategoriesApi

NSString* kJSAPIBRERuleEngineCategoriesApiErrorDomain = @"JSAPIBRERuleEngineCategoriesApiErrorDomain";
NSInteger kJSAPIBRERuleEngineCategoriesApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) deleteBRECategoryTemplateWithId: (NSString*) _id
    ...
    if (_id == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kJSAPIBRERuleEngineCategoriesApiErrorDomain code:kJSAPIBRERuleEngineCategoriesApiMissingParamErrorCode userInfo:userInfo];
            handler(error);
        }
        return nil;
    }
...
///
///  @returns JSAPIPageResourceBreCategoryResource_*
///
-(NSURLSessionTask*) getBRECategoriesWithSize: (NSNumber*) size
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"JSAPIPageResourceBreCategoryResource_*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((JSAPIPageResourceBreCategoryResource_*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) getBRECategoryWithName: (NSString*) name
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"JSAPIBreCategoryResource*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((JSAPIBreCategoryResource*)data, error);
                                }
                            }];
}
