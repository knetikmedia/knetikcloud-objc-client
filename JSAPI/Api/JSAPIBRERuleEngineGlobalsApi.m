#import "JSAPIBRERuleEngineGlobalsApi.h"
...
@interface JSAPIBRERuleEngineGlobalsApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation JSAPIBRERuleEngineGlobalsApi

NSString* kJSAPIBRERuleEngineGlobalsApiErrorDomain = @"JSAPIBRERuleEngineGlobalsApiErrorDomain";
NSInteger kJSAPIBRERuleEngineGlobalsApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) createBREGlobalWithBreGlobalResource: (JSAPIBreGlobalResource*) breGlobalResource
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"JSAPIBreGlobalResource*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((JSAPIBreGlobalResource*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) deleteBREGlobalWithId: (NSString*) _id
    ...
    if (_id == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kJSAPIBRERuleEngineGlobalsApiErrorDomain code:kJSAPIBRERuleEngineGlobalsApiMissingParamErrorCode userInfo:userInfo];
            handler(error);
        }
        return nil;
    }
...
///
///  @returns JSAPIPageResourceBreGlobalResource_*
///
-(NSURLSessionTask*) getBREGlobalsWithFilterSystem: (NSNumber*) filterSystem
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"JSAPIPageResourceBreGlobalResource_*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((JSAPIPageResourceBreGlobalResource_*)data, error);
                                }
                            }];
}
