...
#import "JSAPIJSONRequestSerializer.h"
#import "JSAPIQueryParamCollection.h"
#import "JSAPIDefaultConfiguration.h"

NSString *const JSAPIResponseObjectErrorKey = @"JSAPIResponseObject";

static NSString * const kJSAPIContentDispositionKey = @"Content-Disposition";

static NSDictionary * JSAPI__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * JSAPI__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = JSAPI__headerFieldsForResponse(response);
    if(!headers[kJSAPIContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kJSAPIContentDispositionKey];
...
@interface JSAPIApiClient ()

@property (nonatomic, strong, readwrite) id<JSAPIConfiguration> configuration;

@property (nonatomic, strong) NSArray<NSString*>* downloadTaskResponseTypes;

@end

@implementation JSAPIApiClient

#pragma mark - Singleton Methods

+ (instancetype) sharedClient {
    static JSAPIApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[JSAPIDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[JSAPIDefaultConfiguration sharedConfig]];
}
...
- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<JSAPIConfiguration>)configuration {
    ...
    if (self) {
        ...
        _timeoutInterval = 60;
        _responseDeserializer = [[JSAPIResponseDeserializer alloc] init];
        _sanitizer = [[JSAPISanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        JSAPIJSONRequestSerializer * swgjsonRequestSerializer = [JSAPIJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kJSAPIApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionDataTask*) taskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask *task = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        JSAPIDebugLogResponse(response, responseObject,request,error);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[JSAPIResponseObjectErrorKey] = responseObject;
        }
...
- (NSURLSessionDataTask*) downloadTaskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask* task = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString *filename = JSAPI__fileNameForResponse(response);
...
- (NSURLSessionTask*) requestWithPath: (NSString*) path
    ...
    __weak id<JSAPISanitizer> sanitizer = self.sanitizer;
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = JSAPIPercentEscapedStringFromString(safeString);
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path queryParams:(NSDictionary*) queryParams {
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = JSAPIPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, JSAPIPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[JSAPIQueryParamCollection class]]){
            JSAPIQueryParamCollection * coll = (JSAPIQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = JSAPIPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = JSAPIPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = JSAPIPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)querys WithAuthSettings:(NSArray *)authSettings {
    ...
    id<JSAPIConfiguration> config = self.configuration;
