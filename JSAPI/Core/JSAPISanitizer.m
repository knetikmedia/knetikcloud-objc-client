...
NSString * const kJSAPIApplicationJSONType = @"application/json";

NSString * JSAPIPercentEscapedStringFromString(NSString *string) {
    static NSString * const kJSAPICharactersGeneralDelimitersToEncode = @":#[]@";
    static NSString * const kJSAPICharactersSubDelimitersToEncode = @"!$&'()*+,;=";

    NSMutableCharacterSet * allowedCharacterSet = [[NSCharacterSet URLQueryAllowedCharacterSet] mutableCopy];
    [allowedCharacterSet removeCharactersInString:[kJSAPICharactersGeneralDelimitersToEncode stringByAppendingString:kJSAPICharactersSubDelimitersToEncode]];
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[JSAPIQueryParamCollection class]]) {
        return object;
    }
    ...
    else if ([object isKindOfClass:[JSAPIObject class]]) {
...
- (NSString *) selectHeaderAccept:(NSArray *)accepts {
    ...
    for (NSString *string in accepts) {
        if ([self.jsonHeaderTypeExpression matchesInString:string options:0 range:NSMakeRange(0, [string length])].count > 0) {
            return kJSAPIApplicationJSONType;
        }
