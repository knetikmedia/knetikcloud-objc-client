...
------------- | ------------- | -------------
[**createCustomerUsingPOST**](SWGPaymentsStripeApi.md#createcustomerusingpost) | **POST** /payment/provider/stripe/payment-methods | Create a Stripe payment method for a user
...
// Create a Stripe payment method for a user
[apiInstance createCustomerUsingPOSTWithRequest:request
          completionHandler: ^(SWGPaymentMethodResource* output, NSError* error) {
                        if (output) {
                            NSLog(@"%@", output);
                        }
                        if (error) {
                            NSLog(@"Error calling SWGPaymentsStripeApi->createCustomerUsingPOST: %@", error);
                        }
                    }];
```
